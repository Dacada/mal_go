...
func macroexpand(ast common.MalType, env common.Env) (common.MalType, error) {
	for {
		...
		var err error
		ast, err = fun.Fn.Func(ast_as_list.List[1:])
		if err != nil {
			return nil, err
		}
	}
...
func apply_def(lst []common.MalType, isMacro bool, env common.Env) (common.MalType, error) {
	if len(lst) != 2 {
...
func apply_quote(lst []common.MalType) (common.MalType, error) {
	if len(lst) != 1 {
...
func apply_quasiquote(args []common.MalType) (common.MalType, error) {
	if len(args) != 1 {
		return nil, errors.New("expected one argument to quote")
	}
	ast := args[0]
	...
	ok_array := false
	ast_as_lst, ok := ast.(common.MalTypeList)
	if ok {
		ast_as_arr = ast_as_lst.List
		ok_array = true
	} else {
		ast_as_vec, ok := ast.(common.MalTypeVector)
		ast_as_arr = ast_as_vec.Vector
		is_vec = ok
		ok_array = ok
	}
	if ok_array {
		...
		for i := len(ast_as_arr)-1; i >= 0; i-- {
			...
			if ok {
				if len(elt_as_lst.List) > 0 {
					if elt_as_lst.List[0] == common.MalTypeSymbol("splice-unquote") {
						if len(elt_as_lst.List) == 2 {
							new_res := make([]common.MalType, 3)
							new_res[0] = common.MalTypeSymbol("concat")
							new_res[1] = elt_as_lst.List[1]
							new_res[2] = common.NewMalList(res)
							res = new_res
							continue
						} else {
		...
		if is_vec {
			...
			new_res[1] = common.NewMalList(res)
			res = new_res
		}
		return common.NewMalList(res), nil
	}
...
func apply_macroexpand(ast []common.MalType, env common.Env) (common.MalType, error) {
	if len(ast) > 1 {
...
func apply_dir(ast []common.MalType, env common.Env) (common.MalType, error) {
	if len(ast) > 0 {
...
func apply_try(ast []common.MalType, env common.Env) (common.MalType, error) {
	if len(ast) == 0 {
		return nil, errors.New("expected one or two arguments to try")
	}
	try_form := ast[0]
	res, err := EVAL(try_form, env)
	if err == nil {
		return res, nil
	}
	if len(ast) != 2 {
	...
	catch_ast, ok := ast[1].(common.MalTypeList)
	if !ok {
		return nil, errors.New(fmt.Sprintf("unexpected catch form. expected %T but got %T", catch_ast, ast[1]))
	}
	if len(catch_ast.List) != 3 {
		if len(catch_ast.List) >= 1 && catch_ast.List[0] == common.MalTypeSymbol("catch*") {
			return nil, errors.New("expected three arguments to catch")
		} else {
			return nil, errors.New("invalid form after try form, expected a catch")
		}
	}
	symbol, ok := catch_ast.List[1].(common.MalTypeSymbol)
	if !ok {
		return nil, errors.New(fmt.Sprintf("expected %T as first parameter of catch, got %T", symbol, catch_ast.List[1]))
	}
	catch_form := catch_ast.List[2]
	
	var val common.MalType
	switch e := err.(type) {
	case common.MalTypeError:
		val = e.InnerMalType
	default:
		val = common.MalTypeString(e.Error())
	}

	new_env := common.NewEnv(&env)
	new_env.Set(symbol, val)
	return EVAL(catch_form, new_env)
}
		...
		case common.MalTypeList:
			if len(l.List) == 0 {
				...
				case common.MalTypeSymbol("def!"):
					return apply_def(l.List[1:], false, env)
				case common.MalTypeSymbol("defmacro!"):
					return apply_def(l.List[1:], true, env)
				...
				case common.MalTypeSymbol("quote"):
					return apply_quote(l.List[1:])
				case common.MalTypeSymbol("quasiquote"):
					ast, err = apply_quasiquote(l.List[1:])
					if err != nil {
						return nil, err
					}
					continue
				case common.MalTypeSymbol("quasiquoteexpand"):
					return apply_quasiquote(l.List[1:])
				case common.MalTypeSymbol("macroexpand"):
					return apply_macroexpand(l.List[1:], env)
				case common.MalTypeSymbol("dir"):
					return apply_dir(l.List[1:], env)
				case common.MalTypeSymbol("try*"):
					return apply_try(l.List[1:], env)
				default:
					var loop bool
...
func main() {
	...
	repl_env.Set(common.MalTypeSymbol("eval"), common.NewMalFunction(func(args []common.MalType) (common.MalType, error) {
		if len(args) != 1 {
			return nil, errors.New("expected exactly 1 argument")
		}
		return EVAL(args[0], repl_env)
	}))
	repl_env.Set(common.MalTypeSymbol("*host-language*"), common.MalTypeString("my-go"))
	...
	repl_env.Set(common.MalTypeSymbol("*ARGV*"), common.NewMalList(argv))
