...
func apply_try(ast common.MalTypeList, env common.Env) (common.MalType, error) {
	if len(ast) == 0 {
	...
	if len(catch_ast) != 3 {
		if len(catch_ast) >= 1 && catch_ast[0] == "catch*" {
	...
	symbol, ok := catch_ast[1].(common.MalTypeSymbol)
	if !ok {
		return nil, errors.New(fmt.Sprintf("expected %T as first parameter of catch, got %T", symbol, catch_ast[1]))
	}
	catch_form := catch_ast[2]
